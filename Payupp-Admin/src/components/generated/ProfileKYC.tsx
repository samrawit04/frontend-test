...
export const ProfileKYC = ({
  ...
  const completeSourceOfFunds = () => {
    if (!sourceOfFunds || !employmentStatus || !occupation || !monthlyIncome) {
  ...
  const continueSocialVerification = () => {
    if (!socialMediaLink && !schoolUniversity) {
      setError('Please provide either a social media link or school/university name');
      return;
    }
    setIsLoading(true);
    setError('');
            ...
            <AnimatePresence mode="wait">
              ...
              x: -20
                  ...
                  {socialVerified ? <div className="bg-green-50 border border-green-200 rounded-lg p-6 mb-6">
                      ...
                      <div className="text-left">
                        <label htmlFor="schoolUniversity" className="block text-sm font-medium text-gray-700 mb-2">
                          Which School or University did you attend?
                        </label>
                        <input id="schoolUniversity" type="text" value={schoolUniversity} onChange={e => setSchoolUniversity(e.target.value)} placeholder="Harvard University" className="w-full px-4 py-3 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" />
                      </div>

                      {/* Continue Button for Manual Entry */}
                      {(socialMediaLink || schoolUniversity) && <div className="pt-2">
                          {error && <div className="mb-4 bg-red-50 border border-red-200 rounded-lg p-3 flex items-start gap-2">
                              <AlertCircle size={18} className="text-red-600 shrink-0 mt-0.5" />
                              <p className="text-sm text-red-800">{error}</p>
                            </div>}
