...
export const OfferDetailsView: React.FC<OfferDetailsViewProps> = ({
  ...
  // Calculate based on which field is being edited
  const calculatedReceiveAmount = editMode === 'send' ? parseFloat(sendAmount || '0') * offer.exchangeRate : parseFloat(receiveAmount || '0');
  const calculatedSendAmount = editMode === 'receive' ? parseFloat(receiveAmount || '0') / offer.exchangeRate : parseFloat(sendAmount || '0');
  ...
  const handleProceed = () => {
    const finalSendAmount = editMode === 'receive' ? calculatedSendAmount.toString() : sendAmount;
    if (finalSendAmount && parseFloat(finalSendAmount) > 0) {
      onProceed(finalSendAmount);
    }
  };
        ...
        <div className="bg-white rounded-xl border border-gray-200 shadow-lg p-6">
          ...
          <div className="mb-4">
            ...
            <div className="relative">
              <input type="number" value={editMode === 'send' ? sendAmount : calculatedSendAmount.toFixed(2)} onChange={e => handleSendAmountChange(e.target.value)} placeholder="Enter amount" className="w-full text-2xl font-bold text-gray-900 px-4 py-3 border-2 border-gray-200 rounded-lg focus:border-blue-600 focus:outline-none" />
          ...
          <div className="bg-gray-50 rounded-lg p-3 mb-4 space-y-2">
            <div className="flex justify-between text-sm">
              ...
              <span className="font-semibold text-gray-900">
                {(editMode === 'send' ? parseFloat(sendAmount || '0') : calculatedSendAmount).toLocaleString(undefined, {
            ...
            <div className="pt-2 border-t border-gray-200">
              <div className="flex justify-between items-center mb-2">
                ...
                <span className="text-xl font-bold text-blue-600">
                  {(editMode === 'send' ? calculatedReceiveAmount : parseFloat(receiveAmount || '0')).toFixed(2)} {offer.toCurrency}
                </span>
              </div>
            </div>
          </div>

          {/* Receive Amount - NEW EDITABLE FIELD */}
          <div className="mb-4">
            <label className="block text-sm font-medium text-gray-700 mb-2">
              Or enter amount recipient should get
            </label>
            <div className="relative">
              <input type="number" value={editMode === 'receive' ? receiveAmount : calculatedReceiveAmount.toFixed(2)} onChange={e => handleReceiveAmountChange(e.target.value)} placeholder="Enter recipient amount" className="w-full text-2xl font-bold text-blue-600 px-4 py-3 border-2 border-blue-200 rounded-lg focus:border-blue-600 focus:outline-none bg-blue-50" />
              <div className="absolute right-4 top-1/2 -translate-y-1/2 text-lg font-semibold text-blue-600">{offer.toCurrency}</div>
            </div>
            <p className="text-xs text-gray-500 mt-1">
              {editMode === 'receive' ? `You'll need to send ${calculatedSendAmount.toFixed(2)} ${offer.fromCurrency}` : `Recipient will receive ${calculatedReceiveAmount.toFixed(2)} ${offer.toCurrency}`}
            </p>
          </div>

          {/* CTA Button */}
          <button onClick={handleProceed} disabled={editMode === 'send' && (!sendAmount || parseFloat(sendAmount) <= 0) || editMode === 'receive' && (!receiveAmount || parseFloat(receiveAmount) <= 0)} className="w-full bg-blue-600 text-white font-semibold py-3 rounded-lg hover:bg-blue-700 transition-colors disabled:bg-gray-300 disabled:cursor-not-allowed flex items-center justify-center gap-2">
