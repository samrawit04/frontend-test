...
import { User, Mail, Phone, MapPin, Calendar, Shield, CheckCircle2, XCircle, Clock, Star, TrendingUp, Award, FileText, Upload, AlertCircle, DollarSign, Users, Building2, X, ChevronRight, BadgeCheck, Edit2, Camera, ArrowRight, Receipt } from "lucide-react";
...
export default function ProfilePage({
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'verification' | 'reviews' | 'qualifications' | 'transactions'>('overview');
      ...
      <div className="max-w-6xl mx-auto">
                  ...
                  <div>
                    ...
                    <div className="mb-2">
                      <p className="text-sm text-gray-700">
                        <span className="font-semibold">Alias:</span> Sankofa
                      </p>
                      <p className="text-xs text-gray-500 italic">
                        (We do not share your actual name and details with anyone on the platform. You have complete privacy.)
                      </p>
                    </div>
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-gray-200 overflow-hidden mb-6">
          ...
          <div className="p-6">
            ...
            {/* Transactions Tab (for both users and agents) */}
            {activeTab === 'transactions' && <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
                  <div className="p-4 bg-green-50 border border-green-200 rounded-lg">
                    <p className="text-sm text-green-700 mb-1">Total Completed</p>
                    <p className="text-2xl font-bold text-green-900">24</p>
                  </div>
                  <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg">
                    <p className="text-sm text-blue-700 mb-1">Total Volume</p>
                ...
                fromCountry: '🇬🇭 Ghana',
                toCountry: '🇬🇧 UK'
              }].map(transaction => <div key={transaction.id} className="p-4 bg-white border border-gray-200 rounded-lg hover:border-blue-300 hover:shadow-md transition-all">
                      <div className="flex items-start justify-between mb-3">
                        <div className="flex-1">
                          <div className="flex items-center gap-2 mb-1">
                            <p className="font-semibold text-gray-900">{transaction.recipient}</p>
                            <span className={`px-2 py-0.5 text-xs font-medium rounded ${transaction.status === 'completed' ? 'bg-green-100 text-green-700' : transaction.status === 'pending' ? 'bg-yellow-100 text-yellow-700' : 'bg-red-100 text-red-700'}`}>
                              {transaction.status.charAt(0).toUpperCase() + transaction.status.slice(1)}
                            </span>
                          </div>
                          <div className="flex items-center gap-2 text-sm text-gray-600 mb-2">
                            <span>{transaction.fromCountry}</span>
                            <ArrowRight size={14} />
                            <span>{transaction.toCountry}</span>
                          </div>
                          <p className="text-xs text-gray-500">
                            {transaction.id} • {new Date(transaction.date).toLocaleDateString()} • {transaction.method}
                          </p>
                        </div>
                ...
                <div className="text-center py-12 text-gray-400 hidden">
                  <Receipt size={48} className="mx-auto mb-3 opacity-50" />
                  <p className="text-lg font-medium text-gray-600 mb-1">No transactions yet</p>
